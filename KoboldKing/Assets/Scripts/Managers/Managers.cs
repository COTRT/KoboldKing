    ...
    public class Managers : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            var prioritizedGroups = from a in AppDomain.CurrentDomain.GetAssemblies() //Get Every Assembly (like System.Linq and KoboldKing) in the current Application Domain
                                    ...
                                    where !t.IsAbstract && typeof(IGameManager).IsAssignableFrom(t) && typeof(ScriptableObject).IsAssignableFrom(t) && t.GetConstructor(Type.EmptyTypes) != null //Filter down to only types that a) aren't abstract, b) are IGameManager's, and c) can be constructed (as in new Manager()) without any parameters
        ...
        private IEnumerator StartupManagers(IEnumerable<IGrouping<int, Type>> prioritizedTypes)
        {
            ...
            foreach (var pGroup in prioritizedTypes)
            {
                ...
                {
                    IGameManager m;
                    if(typeof(MonoBehaviour).IsAssignableFrom(mType))
                    {
                        m = (IGameManager)gameObject.AddComponent(mType);
                    }
                    else
                    {
                        m = (IGameManager)ScriptableObject.CreateInstance(mType);
                    }
